import os
import requests as req
from flask import Blueprint, jsonify, request
from flask_jwt_extended import jwt_required, get_jwt_identity
from api.models import db
from api.models.wallet import MetaApiAccount

wallet_bp = Blueprint('wallet', __name__, url_prefix='/wallet')

METAAPI_BASE_URL = "https://mt-provisioning-api-v1.agiliumtrade.agiliumtrade.ai"
METAAPI_PROVISIONING_URL = f"{METAAPI_BASE_URL}/users/current/accounts"


def _metaapi_headers():
    token = os.getenv("METAAPI_TOKEN", "")
    return {"auth-token": token, "Content-Type": "application/json"}


@wallet_bp.route('/servers', methods=['GET'])
@jwt_required()
def search_servers():
    """
    Proxy to MetaApi known-servers search.
    Query params: query (string), platform (mt4|mt5)
    Returns: { servers: { "Broker Name": ["Server1", "Server2"], ... } }
    """
    query = request.args.get("query", "").strip()
    platform = request.args.get("platform", "mt4").strip().lower()
    version = 5 if platform == "mt5" else 4

    if not query:
        return jsonify({"servers": {}}), 200

    metaapi_token = os.getenv("METAAPI_TOKEN", "")
    if not metaapi_token or metaapi_token == "your_metaapi_token_here":
        return jsonify({"servers": {}}), 200

    try:
        resp = req.get(
            f"{METAAPI_BASE_URL}/known-mt-servers/{version}/search",
            params={"query": query},
            headers={"auth-token": metaapi_token},
            timeout=10,
        )
        if resp.ok:
            return jsonify({"servers": resp.json()}), 200
    except Exception:
        pass

    return jsonify({"servers": {}}), 200


@wallet_bp.route('', methods=['GET'])
@jwt_required()
def get_wallet():
    """Returns the connected MetaApi account for the current user."""
    user_id = int(get_jwt_identity())
    account = MetaApiAccount.query.filter_by(user_id=user_id).first()
    if not account:
        return jsonify({"wallet": None}), 200
    return jsonify({"wallet": account.serialize()}), 200


@wallet_bp.route('/connect', methods=['POST'])
@jwt_required()
def connect_wallet():
    """
    Creates a MetaApi account in DRAFT state (no MT credentials required) under
    the admin's subscription, then returns a configuration link that the end user
    opens on MetaApi's secure page to enter their MT credentials.
    The MT password never touches our server.
    """
    user_id = int(get_jwt_identity())
    data = request.get_json() or {}

    server = data.get("server", "").strip()
    platform = data.get("platform", "mt4").strip().lower()
    name = data.get("name", "").strip() or f"GoldPilot-{server}"
    account_type = data.get("account_type", "demo").strip()

    if not server:
        return jsonify({"description": "server es obligatorio"}), 400

    if platform not in ("mt4", "mt5"):
        return jsonify({"description": "platform debe ser mt4 o mt5"}), 400

    metaapi_token = os.getenv("METAAPI_TOKEN", "")
    if not metaapi_token or metaapi_token == "your_metaapi_token_here":
        return jsonify({"description": "METAAPI_TOKEN no configurado en el servidor"}), 503

    # Remove existing account from MetaApi and DB before creating a new one
    existing = MetaApiAccount.query.filter_by(user_id=user_id).first()
    if existing:
        try:
            req.delete(
                f"{METAAPI_PROVISIONING_URL}/{existing.account_id}",
                headers=_metaapi_headers(),
                timeout=15,
            )
        except Exception:
            pass
        db.session.delete(existing)
        db.session.commit()

    # Create account in DRAFT state — no login/password, so MetaApi returns instantly
    payload = {
        "name": name,
        "server": server,
        "platform": platform,
        "magic": 0,
        "type": "cloud-g2",
    }

    try:
        resp = req.post(
            METAAPI_PROVISIONING_URL,
            json=payload,
            headers=_metaapi_headers(),
            timeout=15,
        )
    except req.exceptions.RequestException as e:
        return jsonify({"description": f"Error al conectar con MetaApi: {str(e)}"}), 502

    if not resp.ok:
        try:
            error_detail = resp.json().get("message", resp.text)
        except Exception:
            error_detail = resp.text
        return jsonify({"description": f"MetaApi error: {error_detail}"}), resp.status_code

    meta_account_id = resp.json().get("id")
    if not meta_account_id:
        return jsonify({"description": "MetaApi no retornó un account ID"}), 502

    # Request a configuration link — user enters MT credentials on MetaApi's secure page
    config_link = ""
    try:
        link_resp = req.put(
            f"{METAAPI_PROVISIONING_URL}/{meta_account_id}/configuration-link",
            headers=_metaapi_headers(),
            timeout=15,
        )
        if link_resp.ok:
            config_link = link_resp.json().get("configurationLink", "")
    except Exception:
        pass  # Non-critical — user can request a new link later

    # Persist to DB (no password stored)
    account = MetaApiAccount(
        user_id=user_id,
        account_id=meta_account_id,
        server=server,
        platform=platform,
        broker_name=name,
        account_type=account_type,
        status="draft",
    )
    db.session.add(account)
    db.session.commit()

    return jsonify({
        "msg": "Cuenta creada. Usa el enlace de configuración para ingresar tus credenciales MT.",
        "wallet": account.serialize(),
        "configuration_link": config_link,
    }), 200


@wallet_bp.route('/config-link', methods=['GET'])
@jwt_required()
def get_config_link():
    """Returns a fresh MetaApi configuration link for the user's existing account."""
    user_id = int(get_jwt_identity())
    account = MetaApiAccount.query.filter_by(user_id=user_id).first()
    if not account:
        return jsonify({"description": "No hay wallet conectada"}), 404

    metaapi_token = os.getenv("METAAPI_TOKEN", "")
    if not metaapi_token or metaapi_token == "your_metaapi_token_here":
        return jsonify({"description": "METAAPI_TOKEN no configurado en el servidor"}), 503

    try:
        link_resp = req.put(
            f"{METAAPI_PROVISIONING_URL}/{account.account_id}/configuration-link",
            headers=_metaapi_headers(),
            timeout=15,
        )
        if not link_resp.ok:
            return jsonify({"description": "MetaApi no pudo generar el enlace"}), 502
        config_link = link_resp.json().get("configurationLink", "")
        return jsonify({"configuration_link": config_link}), 200
    except req.exceptions.RequestException as e:
        return jsonify({"description": f"Error al conectar con MetaApi: {str(e)}"}), 502


@wallet_bp.route('/disconnect', methods=['DELETE'])
@jwt_required()
def disconnect_wallet():
    """Undeploys and removes the MetaApi account, then clears from DB."""
    user_id = int(get_jwt_identity())
    account = MetaApiAccount.query.filter_by(user_id=user_id).first()
    if not account:
        return jsonify({"description": "No hay wallet conectada"}), 404

    # Remove from MetaApi (best effort)
    try:
        req.delete(
            f"{METAAPI_PROVISIONING_URL}/{account.account_id}",
            headers=_metaapi_headers(),
            timeout=15,
        )
    except Exception:
        pass

    db.session.delete(account)
    db.session.commit()
    return jsonify({"msg": "Wallet desconectada"}), 200
